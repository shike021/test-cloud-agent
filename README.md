...
development environment end to end. It is a TypeScript monorepo with two
workspaces:
...
## Available scripts (run from the repository root)
...
- `install`: `npm ci` restores workspace dependencies.
- `terminals`: long-running `api` and `web` dev servers with visible logs.
- `ports`: exposes `4000` (API) and `5173` (web).
